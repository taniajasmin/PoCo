...
function get_society($sname, $show_deleted = false, $with_rev = false)
{
	$query_string = "
	SELECT 	s.*, 
	d.rev_id, 
	...
	if ($with_rev) {
		$query_string .= " AND s.rev_id = d.rev_id";
	}

	$query_string .= " JOIN users r ON r.user_id = d.revised_by
						WHERE s.soc_name =  ?";
    return  (($s = query($query_string,
                            $sname
                        )
            )
